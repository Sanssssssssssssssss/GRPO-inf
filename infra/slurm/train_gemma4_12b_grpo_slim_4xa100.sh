...
#SBATCH --job-name=gemma4-12b-grpo-slim
#SBATCH --nodes=1
...
export COMPACT_SLIM_DATA_ROOT="${COMPACT_SLIM_DATA_ROOT:-$WORK_ROOT/datasets/invoice_reviewer_grpo_strict_evr_720_v1/compact_chat_slim_evr_v1}"
...
RUN_ID="${RUN_ID:-gemma4_12b_grpo_slim_${SLURM_JOB_ID:-manual}}"
CONFIG_PATH="${CONFIG_PATH:-configs/training/gemma4_12b_grpo_slim_canary.json}"
...
if [ -z "${INIT_ADAPTER_PATH:-}" ]; then
  echo "ERROR: set INIT_ADAPTER_PATH or SFT_RUN_ID so GRPO starts from the SFT adapter." >&2
  exit 2
fi
if [ ! -f "$INIT_ADAPTER_PATH/adapter_model.safetensors" ]; then
  echo "ERROR: INIT_ADAPTER_PATH does not contain adapter_model.safetensors: $INIT_ADAPTER_PATH" >&2
  exit 2
fi
...
echo "compact_slim_data_root=$COMPACT_SLIM_DATA_ROOT"
...
cp "$INIT_ADAPTER_PATH/adapter_config.json" "$METADATA_DIR/init_adapter_config.json"
...
python tools/build_slim_evr_dataset.py \
  --source-root "$COMPACT_DATA_ROOT" \
  --output-root "$COMPACT_SLIM_DATA_ROOT" \
  --model-dir "$MODEL_DIR" | tee "$METADATA_DIR/build_slim_evr_dataset.json"
test -f "$COMPACT_SLIM_DATA_ROOT/grpo/prompts_train.jsonl"
find "$COMPACT_SLIM_DATA_ROOT/grpo" -maxdepth 1 -type f -printf '%f\t%s\n' | sort > "$METADATA_DIR/slim_grpo_files.tsv"
cp "$COMPACT_SLIM_DATA_ROOT/slim_evr_dataset_report.json" "$METADATA_DIR/slim_evr_dataset_report.json"
if [ -f "$COMPACT_SFT_DATA_ROOT/sft_compact_dataset_report.json" ]; then
...
python -m grpo_inf.cli audit-dataset \
  --data "$COMPACT_SLIM_DATA_ROOT" \
  --out "$METADATA_DIR/repo_audit_slim.json" \
  --min-cases 500 | tee "$METADATA_DIR/repo_audit_slim.stdout.json"
python - <<'PY' "$METADATA_DIR/slim_evr_dataset_report.json" "$METADATA_DIR/repo_audit_slim.json"
import json
import sys
from pathlib import Path
slim = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8"))
audit = json.loads(Path(sys.argv[2]).read_text(encoding="utf-8"))
overall = slim["overall"]
failures = []
if overall["slim_completion_tokens"]["p95"] > 1200:
    failures.append(f"slim p95 tokens too high: {overall['slim_completion_tokens']['p95']}")
if overall["slim_completion_tokens"]["max"] > 1536:
    failures.append(f"slim max tokens too high: {overall['slim_completion_tokens']['max']}")
if float(overall["exact_gold_total_mean"]) < 0.95:
    failures.append(f"slim report gate failed: exact_gold_total_mean={overall['exact_gold_total_mean']}")
for key in ("exact_gold_schema_valid_rate", "exact_gold_quote_hit_rate"):
    if float(overall[key]) != 1.0:
        failures.append(f"slim report gate failed: {key}={overall[key]}")
if float(overall["exact_gold_total_min"]) < 0.85:
    failures.append(f"slim report gate failed: exact_gold_total_min={overall['exact_gold_total_min']}")
if not audit.get("valid"):
    failures.append("repo audit slim is not valid")
if failures:
    raise SystemExit("; ".join(failures))
print(json.dumps({"valid": True, "slim_completion_tokens": overall["slim_completion_tokens"]}, indent=2))
PY
if [ "${SKIP_LORA_PREFLIGHT:-0}" = "1" ]; then
  python - <<'PY' | tee "$METADATA_DIR/gemma4_lora_preflight.json"
import json
payload = {
    "valid": True,
    "skipped": True,
    "reason": "SKIP_LORA_PREFLIGHT=1; same model/config preflight was already validated before submission.",
}
...
python tools/render_grpo_training_visuals.py \
  --run-dir "$RUN_ROOT" \
  --out-dir "$RUN_ROOT/visualizations/training_diagnostics" | tee "$METADATA_DIR/post_train_visualization_summary.json" || true
