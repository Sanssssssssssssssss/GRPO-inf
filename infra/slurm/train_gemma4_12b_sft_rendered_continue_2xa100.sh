...
#SBATCH --job-name=gemma4-12b-sft-cont
#SBATCH --nodes=1
...
export RENDERED_SFT_DATA_ROOT="${RENDERED_SFT_DATA_ROOT:-$WORK_ROOT/datasets/invoice_reviewer_grpo_strict_evr_720_v1/rendered_sft_chat_v1}"
...
if [ -z "${INIT_ADAPTER_PATH:-}" ]; then
  echo "ERROR: set INIT_ADAPTER_PATH or SFT_RUN_ID for continuation." >&2
  exit 2
fi
...
RUN_ID="${RUN_ID:-gemma4_12b_sft_rendered_continue_200_${SLURM_JOB_ID:-manual}}"
CONFIG_PATH="${CONFIG_PATH:-configs/training/gemma4_12b_sft_rendered_continue_200step.json}"
...
mkdir -p "$HF_HOME" "$HF_HUB_CACHE" "$HF_XET_CACHE" "$HF_DATASETS_CACHE" \
  ...
  "$TORCH_EXTENSIONS_DIR" "$METADATA_DIR" "$RENDERED_SFT_DATA_ROOT"
...
echo "rendered_sft_data_root=$RENDERED_SFT_DATA_ROOT"
...
python tools/build_rendered_sft_dataset.py \
  --source-root "$COMPACT_DATA_ROOT" \
  --output-root "$RENDERED_SFT_DATA_ROOT" \
  --model-dir "$MODEL_DIR" | tee "$METADATA_DIR/rendered_sft_build_report.json"
cp "$RENDERED_SFT_DATA_ROOT/sft_rendered_dataset_report.json" "$METADATA_DIR/sft_rendered_dataset_report.json"
find "$RENDERED_SFT_DATA_ROOT/sft" -maxdepth 1 -type f -printf '%f\t%s\n' | sort > "$METADATA_DIR/rendered_sft_files.tsv"
